import { Client as ZkConfessionBoxClient, type Game } from './bindings';
...
/**
 * Service for interacting with the ZkConfessionBox game contract
 */
export class ZkConfessionBoxService {
  private baseClient: ZkConfessionBoxClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ZkConfessionBoxClient({
  ...
  private createSigningClient(
    ...
    return new ZkConfessionBoxClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ZkConfessionBoxClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
...
// Example: const zkConfessionBoxService = new ZkConfessionBoxService(contractId);
